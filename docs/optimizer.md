...
- When FK→PK alignment is **absent** (no FK, or FK→non-unique) the lookup is **cross** (1:n). Only INNER/CROSS join types qualify; a `left` 1:n chain would need nullable-widened cross-left semantics and is out of scope for v1 (bails, leaving a nested-loop left join). Cross branches are gated by the row/product guards below.
...
- `maxCrossBranchRows` (default 10000) — per-branch row cap for `cross` (1:n) branches; a cross lookup whose estimate exceeds this stays a nested-loop join. At-most-one branches are exempt.
- `maxCrossProduct` (default 1e6) — whole-product cap for a cross fan-out (`outer × Π cross-branch rows`); unknown estimates count as exceeding it.
...
**Cross branches are out of scope.** A node carrying any `cross` (1:n) branch is left serial; the streaming-`cross` + batched combination is owned by `parallel-fanout-lookup-join-cross-mode`. The rule only flips clusters whose branches are all `atMostOne-*`.
...
**Gates** mirror the UNION ALL rule: `concurrencySafe === true` on every branch; the slowest branch's `expectedLatencyMs ≥ gatherThresholdMs` (inert on memory-vtab plans where it is 0); every branch uncorrelated (`isCorrelatedSubquery` false — the driver forks independent contexts). One extra gate is specific to keyed merge: **every key column on every branch must use the binary collation**. The runtime comparator derives solely from branch 0's collations, and the emitter's merged key value is whichever branch's row arrived first (concurrent → non-deterministic); under a non-binary collation that value can diverge from `coalesce`'s left-to-right pick when collation-equal keys are byte-distinct (e.g. NOCASE merging `'A'`/`'a'`). Binary collation keeps equal keys byte-identical so the merged value is well-defined; the rule declines otherwise. (`AsyncGatherNode.validateZipByKey` enforces the weaker *agreement* invariant — and throws on a true mismatch — so manually-constructed non-binary zips are still possible, with the documented non-determinism caveat.)
...
**Out of scope for v1.** Only symmetric `FULL OUTER` chains are recognized (`LEFT`/`RIGHT` outer chains are asymmetric and not zipByKey). Non-canonical projection orderings (key not first, reordered non-key columns, extra/derived columns) are not recognized — a reordering `Project` on top of the gather is future work (`parallel-async-gather-zip-by-key-projection-order`). `USING` / `NATURAL` full joins are untested (the rule keys off an explicit `coalesce` projection over the equated columns). Non-binary key collations are gated out entirely (see Gates above); re-enabling them needs a deterministic merged-key pick in the emitter, tracked by `parallel-async-gather-zip-by-key-nonbinary-collation`.
