...
**Out of scope for v1.** Only symmetric `FULL OUTER` chains are recognized (`LEFT`/`RIGHT` outer chains are asymmetric and not zipByKey). Non-canonical projection orderings (key not first, reordered non-key columns, extra/derived columns) are not recognized — a reordering `Project` on top of the gather is future work (`parallel-async-gather-zip-by-key-projection-order`). `USING` / `NATURAL` full joins are untested (the rule keys off an explicit `coalesce` projection over the equated columns).
