...
That basis covering index can now be kept **write-through current**: an `order by` materialized view declared `with refresh = 'row-time'` is maintained synchronously with each source row-write ([Materialized Views § Row-time refresh](materialized-views.md#row-time-refresh)), which is precisely the consistency a row-time existence lookup needs. The *maintenance* capability for the covering-index shape now exists; *routing* `unique` enforcement through the covering MV's backing table (the in-place `insert or replace` / `or ignore` / `abort` resolution) remains its own downstream ticket (`covering-structure-mv-rowtime-enforcement`).
