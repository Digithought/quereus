...
- Module concurrency contract (`src/vtab/module.ts`, `src/vtab/concurrency.ts`): `VirtualTableModule.concurrencyMode` declares `'serial'` (default), `'reentrant-reads'`, or `'fully-reentrant'`. Memory vtab is `'fully-reentrant'` — its `query()` snapshots an immutable BTree layer at call entry, writes always publish via a fresh layer, and JS's single-thread invariant rules out torn pointer reads. Everything else stays default. Parallel runtime consumers call `getModuleConcurrencyMode(module)` and `acquireConnectionLock(connection)` to fall back to serial behavior when sibling branches share a connection. The lock is not enforced by `ParallelDriver` itself — enforcement belongs in the consumer that owns the vtab interaction (e.g. fan-out lookup join).
