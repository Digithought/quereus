...
- `AsyncGatherNode` (`src/planner/nodes/async-gather-node.ts`): N-ary physical relational combinator that drives ≥ 2 independent children concurrently via `ParallelDriver.drive()`. v1 ships two combinators — `unionAll` (arrival-order interleave) and `crossProduct` (materialized N-ary Cartesian product). Manual-construction only in this commit; recognition of chained `SetOperationNode(op='unionAll')` lands separately in `5.5-parallel-async-gather-union-all-rule`. `crossProduct` recognition is opt-in and not currently on the optimizer roadmap. The third planned combinator `zipByKey` is parked in backlog (`parallel-async-gather-zip-by-key`). See [Runtime § AsyncGatherNode](runtime.md#asyncgathernode-n-ary-parallel-relational-combinator).
