...
- **Assertion delta classification.** Each table reference in an assertion's violation plan is classified as `'row'`, `'group'`, or `'global'`. `'row'` references run a parameterized variant per changed PK (one violation check per touched row). `'group'` references — where an aggregate's `GROUP BY` columns cover a unique key of the underlying table under FD closure — are reserved for parameterized per-group delta evaluation; runtime wiring is deferred and they currently fall back to `'global'` (full violation query). `'global'` references run the full violation query once per impacted assertion. See [Optimizer Documentation: Row‑specific vs Global Classification for Assertions](optimizer.md#rowspecific-vs-global-classification-for-assertions).
