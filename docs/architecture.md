...
*   **Predicate-Driven View Updateability** — views, CTEs, and subqueries-in-`from` are *intended* to be uniformly mutable through `insert` / `update` / `delete`. **Phase 1 shipped** (`view-updateability-phase-1`): single-source projection-and-filter views are writeable — the DML is rewritten to target the base table (`planner/building/view-mutation.ts`) and re-planned, so all constraint / conflict / FK / mutation-context machinery is reused verbatim and reads/writes through the view report the base table to `getChangeScope()` / `Database.watch` for free. Omitted columns are recovered from constant FDs derived from equality selection predicates (`where color = 'green'` defaults `color`) and base-column defaults; `no-inverse` / `predicate-contradiction` / `recursive-cte` and body-shape diagnostics reject what is not handled, and the statement-level `OR` clause routes to the base op. Multi-source fan-out, the broader FD/EC reconstruction chain, `default_for` tags, the `quereus.update.*` override namespace, and the plan-node `updateLineage` / `ViewMutationNode` substrate remain intent (Phase 2+). No `with check option`, no `instead of` triggers, no view-level updateability flag — the FD framework subsumes the dialect-specific rules of other engines. See [View Updateability Documentation](view-updateability.md).
*   **Materialized Views** — `CREATE MATERIALIZED VIEW` stores a query body as a *keyed derived relation*: a hidden backing table primary-keyed by the body's inferred key, addressable like any virtual table, with manual full-refresh (`REFRESH MATERIALIZED VIEW`). Dual-registered (the backing `TableSchema` plus a `MaterializedViewSchema`), read-only to direct DML, and round-tripped through the declarative-schema pipeline (`bodyHash`-keyed rebuild on a body change). See [Materialized Views](materialized-views.md).
*   **Layered Schemas / Lenses** — a database separates into **logical** (embodiment-free design: tables, types, logical constraints, tags — no module, no indexes), **basis** (module-backed relations, possibly federated across modules; covering structures live here as materialized views), and a **mapping** layer of per-logical-table **lenses**. A lens is the bidirectional (`get`/`put`) relational expression realizing a logical table over basis — built on view updateability, so relational algebra is the only operator set in both directions. Lens bodies are authored as **sparse overrides** (rename, hide, compute, …) over a module-pluggable default mapping, merged per-attribute by stable attribute ID and gap-filled by the compiler; the logical spec's constraints are *attached* at the lens boundary (predicates are read-time filters, not invariants), and the lens prover discharges the GetPut/PutGet completeness checks. At deploy the lens compiles to an inline view so the query processor sees an ordinary view; the basis is a generated-then-frozen, hash-coded artifact diffed by the declarative-schema pipeline, with logical removals detaching mappings rather than dropping basis storage. **Phase 1 shipped** (`lens-foundation-and-default-mapper`): the `Schema.kind` discriminator, the `declare logical schema X { ... }` surface, the per-logical-table lens slot, the default name-based aligner (single-source, name-equivalent), deploy-compile at `apply schema X`, and kind-aware diff/hash with asymmetric removal. Pending: explicit `declare lens` overrides + per-attribute merge, the completeness prover + constraint attachment, n-way decomposition, and module mapping advertisements. See [Lenses and Layered Schemas](lens.md).
...
- **Assertion delta classification.** Each table reference in an assertion's violation plan is classified as `'row'`, `'group'`, or `'global'`. All three modes are dispatched by a reusable `DeltaExecutor` kernel at COMMIT time: `'row'` runs a parameterized variant per changed PK; `'group'` parameterizes per changed group key (with OLD/NEW projections for group-membership transitions); `'global'` runs the full violation query once. The same kernel is the integration point for materialized views, reactive signals, and triggers — see [Incremental Maintenance](incremental-maintenance.md) and [Optimizer Documentation](optimizer.md#binding-aware-delta-planning-reusable).
