...
*   **Orthogonal Query Expressions** — `SELECT`, `VALUES`, and DML-with-`RETURNING` are accepted as a single `QueryExpr` shape everywhere a relation is needed (top-level statement, FROM subquery source, scalar / IN / EXISTS subquery, compound legs, CTE bodies, view bodies). The parser flows every relation site through one `parseQueryExpr` helper; the AST collapses `MutatingSubquerySource` into the standard `SubquerySource` and `InsertStmt.{values, select}` into a single `source: QueryExpr`. Today the planner runs the SELECT and VALUES portions at every site; DML in scalar / IN / EXISTS / compound-leg / view-body / non-top CTE-body positions parses but errors cleanly at plan time — the run-once / full-drain / change-scope work that lifts that gate lives in the follow-up ticket `dml-in-expression-position`. See [SQL Reference § Query expressions](sql.md#query-expressions).
