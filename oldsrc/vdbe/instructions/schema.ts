...
export function registerHandlers(handlers: Handler[]) {
  handlers[Opcode.SchemaChange] = async (ctx, inst) => {
    ...
    if (changeInfo === null) {
      throw new QuereusError(`SchemaChange requires valid P4SchemaChange info`, StatusCode.INTERNAL);
    }

    try {
      const cursor = ctx.getCursor(cursorIdx);
      if (!cursor) {
        throw new QuereusError(`SchemaChange: Invalid cursor index ${cursorIdx}`, StatusCode.INTERNAL);
      }

      const vtab = cursor.vtab;
      if (!vtab) {
        throw new QuereusError(`SchemaChange: Cursor ${cursorIdx} does not refer to an open virtual table`, StatusCode.INTERNAL);
      }

      // Check for xAlterSchema on the instance
      if (typeof vtab.xAlterSchema !== 'function') {
        throw new QuereusError(`ALTER TABLE operation not supported by virtual table module for table '${vtab.tableName}'`, StatusCode.MISUSE);
      }
      ...
      ctx.error = new QuereusError(msg, code, e instanceof Error ? e : undefined);
      ctx.done = true;
