...
export function compileColumn(compiler: Compiler, expr: AST.ColumnExpr, targetReg: number, correlation?: SubqueryCorrelationResult, havingContext?: HavingContext, argumentMap?: ArgumentMap): void {
	...
	if (compiler.subroutineDepth > 0 && argumentMap) {
		...
		if (correlatedColInfo) {
			...
			if (argOffset !== undefined) {
				...
				throw new QuereusError(`Internal error: Correlated column ${expr.name} not found in argument map.`, StatusCode.INTERNAL, undefined, expr.loc?.start.line, expr.loc?.start.column);
			}
		}
	}
	...
	if (havingContext) {
		...
		throw new QuereusError(`Column "${expr.name}" must appear in the GROUP BY clause or be used in an aggregate function`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
	}
	...
	if (expr.table) {
		...
		const foundCursor = compiler.tableAliases.get(aliasOrTableName);
		if (foundCursor === undefined) {
			throw new QuereusError(`Table or alias not found: ${expr.table}`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
		}
		cursor = foundCursor;
		tableSchema = compiler.tableSchemas.get(cursor);
		if (!tableSchema) { throw new QuereusError(`Internal error: Schema not found for cursor ${cursor}`, StatusCode.INTERNAL); }
		...
		if (potentialColIdx === undefined) {
			...
			if (expr.name.toLowerCase() === 'rowid') {
				...
				throw new QuereusError(`Column not found in table ${expr.table}: ${expr.name}`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
			}
		} else {
			...
			if (schema) {
				...
				if (idx !== undefined) {
					if (potentialCursor !== -1) {
						throw new QuereusError(`Ambiguous column name: ${expr.name}. Qualify with table name or alias.`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
					}
					...
					if (potentialCursor !== -1) {
						throw new QuereusError(`Ambiguous column name: ${expr.name} (rowid). Qualify with table name or alias.`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
					}
					potentialCursor = cursorId;
					potentialColIdx = -1;
					potentialSchema = schema;
...
export function compileBinary(
	...
	if (expr.right.type === 'subquery' && [
		...
		switch (expr.operator.toUpperCase()) {
			...
			default:
				throw new QuereusError(`Operator '${expr.operator}' cannot be used with a subquery on the right side.`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
		}
	}
	if (expr.left.type === 'subquery' && [
		...
		switch (expr.operator.toUpperCase()) {
			case '=': case '==': case '!=': case '<>': case '<': case '<=': case '>': case '>=':
				compiler.compileComparisonSubquery(expr.left, expr.operator, subQuery, targetReg); return;
			default:
				throw new QuereusError(`Operator '${expr.operator}' cannot be used with a subquery on the left side.`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
		}
	}
	...
	switch (expr.operator.toUpperCase()) {
		...
		case 'LIKE': case 'GLOB': {
			throw new QuereusError(`Operator ${expr.operator} not fully implemented yet with collation support.`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
		}
		case 'AND': {
		...
		default:
			throw new QuereusError(`Unsupported binary operator: ${expr.operator}`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
	}
}
...
export function compileUnary(
	...
	switch (expr.operator.toUpperCase()) {
		...
		default: throw new QuereusError(`Unsupported unary operator: ${expr.operator}`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
	}
}
...
export function compileFunction(compiler: Compiler, expr: AST.FunctionExpr, targetReg: number, correlation?: SubqueryCorrelationResult, havingContext?: HavingContext, argumentMap?: ArgumentMap): void {
	...
	if (!funcDef) { throw new QuereusError(`Function not found: ${expr.name}/${expr.args.length}`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column); }
	...
	if (isAggregate && !havingContext) {
		...
		throw new QuereusError(`Aggregate function ${funcDef.name} used incorrectly in HAVING clause.`, StatusCode.ERROR, undefined, expr.loc?.start.line, expr.loc?.start.column);
	} else {
		throw new QuereusError(`Invalid function call context for ${funcDef.name}`, StatusCode.INTERNAL, undefined, expr.loc?.start.line, expr.loc?.start.column);
	}
}
