...
function generateVdbeForStep(
	...
	if (step.type === 'Scan') {
		...
		if (plan.idxNum !== 0) {
			...
			plan.usage.forEach((usage: { argvIndex: number; omit: boolean }, constraintIdx: number) => {
				if (usage.argvIndex > 0) {
					...
					if (!expr) throw new QuereusError(`Internal error: Missing expression for constraint ${constraintIdx} in Scan step`, StatusCode.INTERNAL);
		...
		if (outerStepIndex === -1 || innerStepIndex === -1) {
			throw new QuereusError(`Internal error: Could not find outer/inner step index for join ${outerAlias}<->${innerAlias}`, StatusCode.INTERNAL);
		}
		...
		if (!outerRelation) throw new QuereusError(`Internal error: Could not determine outer relation for join step`, StatusCode.INTERNAL);
		...
		if (!innerRelation) throw new QuereusError(`Internal error: Could not determine inner relation for join step`, StatusCode.INTERNAL);
