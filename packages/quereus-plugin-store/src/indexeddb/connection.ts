/**
 * VirtualTableConnection implementation for IndexedDB tables.
 *
...
import type { TransactionCoordinator } from '../common/transaction.js';

let connectionCounter = 0;

/**
 * Connection to an IndexedDB-backed table.
 * All connections share a TransactionCoordinator for multi-table atomicity.
 */
export class IndexedDBConnection implements VirtualTableConnection {
  public readonly connectionId: string;
  public readonly tableName: string;
  private coordinator: TransactionCoordinator;

  constructor(tableName: string, coordinator: TransactionCoordinator) {
    this.connectionId = `indexeddb-${tableName}-${++connectionCounter}`;
