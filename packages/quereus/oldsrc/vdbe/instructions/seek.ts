...
export function registerHandlers(handlers: Handler[]) {
  handlers[Opcode.SeekRelative] = async (ctx, inst) => {
    ...
    if (!cursor?.instance) {
      throw new QuereusError(`SeekRelative: Invalid or unopened cursor index ${cIdx}`, StatusCode.INTERNAL);
    }
    if (typeof cursor.instance.seekRelative !== 'function') {
        throw new QuereusError(`SeekRelative not supported by cursor ${cIdx}`, StatusCode.MISUSE);
    }
    ...
    if (typeof offsetValue === 'number') {
      ...
      throw new QuereusError(`SeekRelative: Offset value must be a number or bigint, got ${typeof offsetValue} (cursor ${cIdx})`, StatusCode.INTERNAL);
    }
  ...
  handlers[Opcode.SeekRowid] = async (ctx, inst) => {
    ...
    const cursor = ctx.getCursor(cIdx);
    if (!cursor?.instance) {
        throw new QuereusError(`SeekRowid: Invalid or unopened cursor index ${cIdx}`, StatusCode.INTERNAL);
    }
    if (typeof cursor.instance.seekToRowid !== 'function') {
        throw new QuereusError(`SeekRowid not supported by cursor ${cIdx}`, StatusCode.MISUSE);
    }
    ...
    if (typeof rowidValue === 'bigint') {
      ...
      throw new QuereusError(`SeekRowid: Target rowid must be an integer or bigint, got ${typeof rowidValue} (cursor ${cIdx})`, StatusCode.INTERNAL);
    }
