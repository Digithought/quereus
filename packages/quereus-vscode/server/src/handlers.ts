/**
 * Pure handler functions for the Quereus LSP server.
 * No side effects, no LSP connection dependency — testable in isolation.
 */
import {
	Diagnostic,
	DiagnosticSeverity,
...
export const SQL_KEYWORDS = Object.keys(KEYWORDS);

export const tokenTypes = ['keyword', 'function', 'string', 'number', 'operator', 'variable', 'comment'] as const;
export type TokenTypeLabel = typeof tokenTypes[number];
export const tokenTypeToIndex: Record<TokenTypeLabel, number> =
	Object.fromEntries(tokenTypes.map((t, i) => [t, i])) as Record<TokenTypeLabel, number>;
...
export function getCompletions(
	...
	externalSchema: SchemaSnapshot | null,
	keywords: string[],
): CompletionItem[] {
	const items: CompletionItem[] = keywords.map(k => ({ label: k, kind: CompletionItemKind.Keyword }));
...
export function toRange(loc: { start: { line: number; column: number }; end: { line: number; column: number } }) {
	return {
...
export function computeDiagnostics(text: string, Parser: new () => ParserType): Diagnostic[] {
...
export interface RawToken { start: number; end: number; type: TokenTypeLabel }

export function sortAndMergeSpans(spans: Span[]): Span[] {
...
export function isInsideSortedSpans(offset: number, spans: Span[]): boolean {
...
/** Tokenize SQL text into classified spans. Pure function — no LSP dependency. */
export function tokenize(text: string, keywords: string[]): RawToken[] {
...
/** Convert raw tokens into LSP SemanticTokens, handling multiline spans. */
