...
export function emitParameterReference(plan: ParameterReferenceNode): Instruction {
	function run(ctx: RuntimeContext): SqlValue {
		...
		if (ctx.params === undefined) {
			throw new SqliterError('Query executed with parameters, but no parameter values were provided.', StatusCode.MISUSE);
		}
		...
		if (typeof identifier === 'number') {
			...
			if (Array.isArray(ctx.params)) {
				if (identifier < 1 || identifier > ctx.params.length) {
					throw new SqliterError(`Parameter index ${identifier} is out of bounds.`, StatusCode.RANGE);
				}
				...
				if (!(key in ctx.params)) {
					throw new SqliterError(`Parameter with index ${identifier} not found in provided object.`, StatusCode.NOTFOUND);
				}
				return ctx.params[key as keyof typeof ctx.params];
			} else {
				throw new SqliterError('Parameters provided in an unsupported format for indexed access.', StatusCode.MISUSE);
			}
		} else if (typeof identifier === 'string') {
			// For named parameters like :name.
			if (Array.isArray(ctx.params)) {
				throw new SqliterError('Named parameter found in query, but parameters provided as an array.', StatusCode.MISUSE);
			}
			if (typeof ctx.params === 'object' && ctx.params !== null) {
				const key = identifier.startsWith(':') ? identifier.substring(1) : identifier;
				if (!(key in ctx.params)) {
					throw new SqliterError(`Parameter with name '${key}' not found.`, StatusCode.NOTFOUND);
				}
				return ctx.params[key as keyof typeof ctx.params];
			} else {
				throw new SqliterError('Parameters provided in an unsupported format for named access.', StatusCode.MISUSE);
			}
		} else {
			// Should not happen given ParameterReferenceNode structure
			throw new SqliterError('Invalid parameter identifier type.', StatusCode.INTERNAL);
		}
	}
