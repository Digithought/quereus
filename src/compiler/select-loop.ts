...
import type { ColumnResultInfo } from './compiler.js'; // Import ColumnResultInfo
...
export type ProcessRowCallback = (
	...
	plannedStepsContext: PlannedStep[], // Changed from ReadonlyArray
	activeOuterCursors: ReadonlySet<number>,
	context: RowProcessingContext // CHANGED: Pass context object
...
interface VdbeLoopInfo {
	...
	callbackAddr?: number; // Address of the start of the row processing logic (needed for padding)
...
function generateVdbeForStep(
	...
	if (step.type === 'Scan') {
		...
		if (plan.idxNum !== 0) {
            ...
            plan.usage.forEach((usage, constraintIdx) => {
        ...
        if (isInnermostStep) {
            ...
            callbackAddr = processRowCallback(compiler, null, plannedSteps, currentActiveCursors, processingContext);
             ...
             throw new SqliteError("Internal error: Scan step encountered that is not the innermost step.", StatusCode.INTERNAL);
             // If scans could appear mid-plan (e.g., subquery scans), this would need adjustment.
        }
		...
		if (joinStep.joinType === 'left' && joinStep.matchReg !== undefined) {
            ...
            if (isInnermostStep) {
                ...
                innerRelation.schema.columns.forEach((colSchema) => {
                    if (!colSchema.hidden) {
                        warnLog(`LEFT JOIN NULL padding for intermediate join ${innerAlias}.${colSchema.name} relies on downstream handling.`);
                        // The proper handling here would be to identify which registers in the deeper
                        // nested joins need to be NULL-filled based on this relation.
                        // For now, we'll emit placeholder NULLs and warn.
...
export function compilePlannedStepsLoop(
	...
	processRowCallback: ProcessRowCallback,
	finalColumnMap: ReadonlyArray<ColumnResultInfo> = [] // Added parameter for column mapping
