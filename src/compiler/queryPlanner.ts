...
export function planTableAccessHelper(
	...
	log(`Plan: %s (cursor %d, outer: %s) -> idxNum=%d cost=%.2f rows=%s usage=%j handled=%d colUsed=%s`, tableSchema.name, cursorIdx, [...activeOuterCursors].join(','), planResult.idxNum, planResult.cost, planResult.rows.toString(), planResult.usage, planResult.handledWhereNodes.size, colUsed.toString(2));
}
