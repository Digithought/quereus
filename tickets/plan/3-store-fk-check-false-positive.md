...
prereq: store-transaction-isolation-and-rollback (still in tickets/fix/, not yet landed)
files:
...
## Blocked on: `store-transaction-isolation-and-rollback`
...
This is the same visibility gap described in section A ("Reads inside a transaction don't see the transaction's own writes") of `store-transaction-isolation-and-rollback`. Memory mode passes because its VTab reads reflect pending writes.
...
Unblock: once `store-transaction-isolation-and-rollback` is in `review/` or `complete/`, re-run `yarn test:store` on `41-foreign-keys.sqllogic`; either move this to `complete/` (resolved-by-dependency, with a confirming store spec added) or to `implement/` with a narrowed scope (evaluator read-path bypass of the overlay).
